...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        */
        String authHeader = request.getHeader("Authorization"); // Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
        String token = null;
        String username = null;

        // let's check if header contains a JWT
        if(authHeader != null && authHeader.startsWith("Bearer ")) {
            token = authHeader.substring(7);    // to take the token string after the string 'Bearer '
        ...
        if(username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if(jwtUtil.validateToken(token)) {  // if the token is invalid then no authentication is set, request continues as unauthenticated
                ...
                // that this this request is from user: "username" with roles: ROLE_USER, etc.
